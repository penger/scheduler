...
<script type="text/javascript">
	...
	function addRelation(type,source_id){
		...
		$.ajax({
			url:"./relation?op=addRelation&type="+type+"&source_id="+source_id+"&etl_id="+etl_id ,
			type:"GET",
			dataType : "html",
			async : true, // 同步
			data : '',
			success:function(response){
				$("#message").append("增加"+response+"条记录 ! 刷新查看<br/>")
			}
		})
			
	}
	//删除关联关系
	function deleteRelation(type,source_id){
		
		var etl_id=${etl_id}
		//跳到添加关系的页面,再冲添加关系的页面返回到当前页面
		$.ajax({
			url:"./relation?op=deleteRelation&type="+type+"&source_id="+source_id+"&etl_id="+etl_id ,
			type:"GET",
			dataType : "html",
			async : true, // 同步
			data : '',
			success:function(data){
				if(data!=0){
					alert("删除"+data+"条记录 ! 刷新查看")
					$("#tr_"+source_id).remove();
				}else{
					alert("删除失败")
				}
			...
			$.ajax({
				...
				success:function(response){
					for(var i=0;i<response.list.length;i++){
						...
						$("#append").append(tid+"-->"+tn+"<a href='javascript:addRelation(0,"+tid+")'>增加</a><br/>")
					}
				}
			})
		})
			...
			$.ajax({
				...
				success:function(response){
					for(var i=0;i<response.list.length;i++){
						...
						$("#append2").append(tid+"-->"+tn+" <a href='javascript:addRelation(1,"+tid+")'>add</a><br/>")
					}
				}
			})
		})
...
<body>
	<div id="message">${etl_id } --- > ${hive_table }</div>
	<div id="s2e">
		依赖的源表为:<br />
		...
		<table border="1" id="s2e">
			...
			<c:forEach items="${source2etl }" var="item" varStatus="status">
				<tr id="tr_${item.source_id }">
					<td >${item.source_id }</td>
					<td >${item.hive_table_name }</td>
					<td><a href="javascript:deleteRelation(0,${item.source_id })">删除</a></td>
				</tr>
			</c:forEach>
		</table>
	</div>
	...
	<div id="e2e">
		...
		<table border="1" id="tasktable">
			...
			<c:forEach items="${etl2etl }" var="item" varStatus="status">
				<tr id="tr_${item.etl_id }">
					<td>${item.etl_id }</td>
					<td><a href=javascript:updateETL('${item.hive_table }',${item.etl_id })>${item.hive_table }</a></td>
					<td><a href="javascript:deleteRelation(1,${item.etl_id })">删除</a></td>
				</tr>
			</c:forEach>
		</table>
	</div>
